/**
 * Manager centre — self-study materials upload (Phase S7).
 */
(function () {
  ...
  function renderTable(tbody, emptyEl) {
    const api = MAT();
    if (!api || !tbody) return;
    const items = api.readAll();
    tbody.innerHTML = "";
    ...
    items.forEach((item) => {
      ...
      tr.innerHTML = `
        ...
        <td>${api.escapeHtml(item.fileName || item.url || "—")}</td>
